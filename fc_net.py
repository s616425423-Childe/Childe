...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params["W2"]
        b2 = self.params["b2"]
        # 网络结构：X → Affine → ReLU → Affine → scores
        ...
        # 简介正则化d/dW ( 0.5 * lambda * W^2 ) = lambda * W
        loss += 0.5 * self.reg * (
                np.sum(W1 * W1)
                + np.sum(W2 * W2)
        )
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for param_name in self.params:
            self.params[param_name] = self.params[
                param_name
            ].astype(dtype)
        ############################################################################
        #                             END OF YOUR CODE                             #
    ...
    def loss(self, X, y=None):
        ...
        scores, final_cache = affine_forward(
            out,
            self.params[f"W{last_layer}"],
            self.params[f"b{last_layer}"]
        )
        ...
        grads[f"W{last_layer}"] = (
                dW
                + self.reg
                * self.params[f"W{last_layer}"]
        )

        grads[f"b{last_layer}"] = db
